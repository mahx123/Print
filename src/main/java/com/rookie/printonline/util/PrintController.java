...
public class PrintController {
    ...
    public void generateAndPrint(PrintData data) {
        try {
            // 生成二维码图片
            BufferedImage qrImage = LabelPrinter.generateQRCode(
                    data.getQrcodeContent(), 100);

            // 准备右侧文本
            String rightText = data.getRightSideText();

            // 创建打印任务
            LabelPrinter printer = new LabelPrinter(qrImage, rightText);
            printer.print();
